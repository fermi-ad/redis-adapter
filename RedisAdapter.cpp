...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  RA_Time : constructor that converts an id string to nanoseconds and sequence
//  number
//
//    id     : Redis ID string e.g. "12345-67089" where the first number is
//    milliseconds since
//             epoch and the second number has nanoseconds remainder scaled by
//             1E10 added to a sequence number, the scale factor is chosen as a
//             power of ten to make the remainder and sequence number human
//             readable in the id string
//    return : RA_Time
//
...
string RA_Time::id() const {
  //  find the remainder nanoseconds (i.e. remove all the whole milliseconds)
  //  and place that at the REMAINDER_SCALE position in the base-ten
  //  representation of the number, then place the seqnum at the ones position
  //  in the base-ten representation of the number - this way the values will be
  //  human-readable
  uint64_t mixed = (nanos % NANOS_PER_MILLI) * REMAINDER_SCALE + seqnum;

  //  place the whole milliseconds on the left-hand side of the ID and the
  //  number from above on the right-hand side of the ID
...
//    options : struct of default values, override using per-field initializer
//    list
//              e.g. { .user = "adinst", .password = "adinst" }
//    return  : RedisAdapter
//
RedisAdapter::RedisAdapter(const string& baseKey,
...
//
RA_Time RedisAdapter::addSingleDouble(const string& subKey, double data,
                                      const RA_ArgsAdd& args) {
  string key = build_key(subKey);
  Attrs attrs = default_field_attrs(data);

  string id = args.trim ? _redis.xaddTrim(key, args.time.id_or_now(),
                                          attrs.begin(), attrs.end(), args.trim)
                        : _redis.xadd(key, args.time.id_or_now(), attrs.begin(),
                                      attrs.end());
  connect(id.size());
  return RA_Time(id);
}
...
//
bool RedisAdapter::subscribe(const string& subKey, ListenSubFn func,
...
//    return  : true if listener started, false if listener failed to start
//
bool RedisAdapter::psubscribe(const string& subKey, ListenSubFn func,
                              const string& baseKey) {
  //  don't allow psubscribe if wildcards in the base key
  if (baseKey.size()) {
    if (baseKey.find_first_of("*?[]") != string::npos) {
      return false;
    }
  } else {
    if (_base_key.find_first_of("*?[]") != string::npos) {
      return false;
    }
  }
...
bool RedisAdapter::unsubscribe(const string& subKey, const string& baseKey) {
  ...
  return (_pattern_subs.size() || _command_subs.size()) ? start_listener()
                                                        : true;
}

//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  setDeferReaders : defer or un-defer addition and removal of readers
//                    - deferring cancels all reads and stops all reader threads
//                    until un-defer
//                    - un-deferring starts all reader threads
//                    this prevents redundant thread destruction/creation and is
//                    the preferred way to add/remove multiple readers at one
//                    time
//
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  addGenericReader : add a reader for a key that does NOT follow RedisAdapter
//  schema
//
//    key     : the key to add (must NOT be a RedisAdapter schema key)
//    func    : function to call when data is read - data will be
//    RedisAdapter::Attrs return  : true if reader started, false if reader
//    failed to start
//
bool RedisAdapter::addGenericReader(const string& key,
                                    ReaderSubFn<Attrs> func) {
  if (split_key(key).first.size())
    return false;  //  reject if RedisAdapter key found
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  removeGenericReader : remove all readers for a key that does NOT follow
//  RedisAdapter schema
//
...
//  Private methods
//
string RedisAdapter::build_key(const string& subKey,
                               const string& baseKey) const {
  //  surround base key with {} to locate keys with same base key in same
  //  cluster slot this mitgates CROSSSLOT errors for copyKey and renameKey but
  //  also puts all keys for a base key onto the same reader thread (this could
  //  be mitigated with an additional load balancing strategy of mutiple threads
  //  per slot if necessary) NOTE - none of this has ANY effect for single
  //  instance (non-cluster) Redis servers
  return "{" + (baseKey.size() ? baseKey : _base_key) + "}" +
         (subKey.size() ? ":" + subKey : "");
}
...
bool RedisAdapter::copy(const string& srcSubKey, const string& dstSubKey,
                        const string& baseKey) {
  ...
  //  WARNING - this cross-slot copy brings ALL the data from srcKey to the
  //  client computer for
  //            manual re-add to dstKey - this is potentially network, memory
  //            and cpu intensive!
  if (ret == -2 && _redis.exists(dstKey) == 0) {
    ...
    if (_redis.xrange(srcKey, "-", "+", back_inserter(raw))) {
      string id;
      for (auto& it : raw) {
        id = _redis.xadd(dstKey, it.first, it.second.begin(), it.second.end());
      }
      ret = id.size();
    }
  }
...
bool RedisAdapter::start_listener() {
  ...
  unique_lock<mutex> lk(mx,
                        defer_lock);  //  thread is about to enter consume loop
  ...
  _listener = thread([&]() {
    ...
    for (const auto& cs : _command_subs) {
      psub->subscribe(cs.first);
    }

    for (const auto& ps : _pattern_subs) {
      psub->psubscribe(ps.first);
    }
...
bool RedisAdapter::add_reader_helper(const string& baseKey,
                                     const string& subKey, reader_sub_fn func) {
  string key = build_key(subKey, baseKey);
  int32_t slot = _redis.keyslot(key);
  if (slot < 0) return false;
...
bool RedisAdapter::remove_reader_helper(const string& baseKey,
                                        const string& subKey) {
  string key = build_key(subKey, baseKey);
  int32_t slot = _redis.keyslot(key);
  if (slot < 0 || _reader.count(slot) == 0) return false;
...
bool RedisAdapter::start_reader(uint16_t slot) {
  ...
  info.thread = thread([&]() {
    ...
    for (Streams out; info.run; out.clear()) {
      if (_redis.xreadMultiBlock(info.keyids.begin(), info.keyids.end(),
                                 _options.timeout, inserter(out, out.end()))) {
        for (auto& item : out) {
          if (item.second.size()) {
            info.keyids[item.first] = item.second.back().first;

            //  when the first result with an id comes back set all '$' to that
            //  id this prevents missing other results on '$' while processing
            //  this one
...
bool RedisAdapter::stop_reader(uint16_t slot) {
  ...
  connect(
      _redis.xaddTrim(info.stop, "*", attrs.begin(), attrs.end(), 1).size());
  info.thread.join();
  return true;
}
