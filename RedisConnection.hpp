...
//
//  Provides common interface to either a Redis single server or a Redis server
//  cluster The user will not know which server type is connected If an
//  exception is thrown in a method, the exception is logged and failure is
//  returned If a method is called while not connected, failure is returned but
//  not logged
//
class RedisConnection {
 public:
  ...
  RedisConnection(const Options& opts) {
    if (!connect(opts))
      syslog(LOG_ERR, "RedisConnection failed to connect in constructor");
  }
  ...
  bool connect(const Options& opts) {
    ...
    if (is_unix_socket) {
      co.type =
          swr::ConnectionType::UNIX;  // Set the connection type to UNIX socket
      co.path = opts.host;            // Set the Unix socket path
    } else {
      co.host = opts.host;
      if (opts.user.size())
        co.user = opts.user;  //  dont overwrite redis++ default with ""
      co.password = opts.password;
      co.socket_timeout = chr::milliseconds(opts.timeout);
      if (opts.port)
        co.port = opts.port;  //  dont overwrite redis++ default with 0
    }
    cpo.size = opts.size;

    try {
      _cluster = std::make_unique<swr::RedisCluster>(co, cpo);
    }  //  this one throws
    catch (...) {
      try {
        ...
        _singler.reset();
      }  //  reset _singler to null since not really connected
    }
    //  a live server is connected, either _cluster OR _singler is valid (but
    //  not both)
    ...
    if (is_unix_socket)
      ...
      syslog(LOG_ERR, "RedisConnection can't connnect to %s:%i",
             co.host.c_str(), co.port);
    return false;
  }
  ...
  bool ping(const std::string& key = "ping") {
    try {
      if (_cluster)
        return _cluster->redis(key, false).ping().compare("PONG") == 0;
  ...
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //  xrange : read a forward-id-ordered (newest last) list of elements from a
  //  stream
  //
  ...
  bool xrange(const std::string& key, const std::string& beg,
    ...
    try {
      if (_cluster) {
        _cluster->xrange(key, beg, end, cnt, out);
        return true;
      }
      if (_singler) {
        _singler->xrange(key, beg, end, cnt, out);
        return true;
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
  ...
  bool xrange(const std::string& key, const std::string& beg,
    ...
    try {
      if (_cluster) {
        _cluster->xrange(key, beg, end, out);
        return true;
      }
      if (_singler) {
        _singler->xrange(key, beg, end, out);
        return true;
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
  ...
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //  xrevrange : read a reverse-id-ordered (newest first) list of elements from
  //  a stream
  //
  //    key    : the stream to read
  //    end    : the highest id to read
  ...
  bool xrevrange(const std::string& key, const std::string& end,
    ...
    try {
      if (_cluster) {
        _cluster->xrevrange(key, end, beg, cnt, out);
        return true;
      }
      if (_singler) {
        _singler->xrevrange(key, end, beg, cnt, out);
        return true;
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
  ...
  bool xrevrange(const std::string& key, const std::string& end,
    ...
    try {
      if (_cluster) {
        _cluster->xrevrange(key, end, beg, out);
        return true;
      }
      if (_singler) {
        _singler->xrevrange(key, end, beg, out);
        return true;
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
  ...
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  //  xreadMultiBlock : read from multiple streams, block until new data on one
  //  or more streams
  //
  //    fst    : the first element of map<string, string> of: stream key -> most
  //    recent element id read lst    : the last element of map<string, string>
  //    of: stream key -> most recent element id read tmo    : timeout in
  //    milliseconds (zero means block indefinitely) out    : the elements read
  //    as Streams per https://github.com/sewenew/redis-plus-plus#examples-4
  //    return : true if connected
  //             false if not connected
  //
  //  Note that this method will fail on a cluster unless the specified keys all
  ...
  bool xreadMultiBlock(Input fst, Input lst, uint32_t tmo, Output out) {
    try {
      if (_cluster) {
        _cluster->xread(fst, lst, chr::milliseconds(tmo), out);
        return true;
      }
      if (_singler) {
        _singler->xread(fst, lst, chr::milliseconds(tmo), out);
        return true;
      }
  ...
  //    id     : the id for the element, must exceed current highest id, "*"
  //    means server will generate fst    : the first element of map<string,
  //    string> of: field -> value fst    : the last element of map<string,
  //    string> of: field -> value return : the id of the new element if
  //    successful
  //             empty string if unsuccsessful or not connected
  //
  template <typename Input>
  std::string xadd(const std::string& key, const std::string& id, Input fst,
                   Input lst) {
    try {
  ...
  //
  //    key    : the stream to add an element to
  //    thr    : the threshold (number of elements) to trim stream to (zero
  //    works but is silly) apx    : true if thr can be approximate (>=), false
  //    if thr should be exact return : the number of trimmed elements if
  //    successful
  ...
  //    means server will generate fst    : the first element of map<string,
  //    string> of: field -> value fst    : the last element of map<string,
  //    string> of: field -> value thr    : the threshold (number of elements)
  //    to trim stream to (zero works but is silly) apx    : true if thr can be
  //    approximate (>=), false if thr should be exact return : the id of the
  //    new element if successful
  //             empty string if unsuccsessful or not connected
  //
  template <typename Input>
  std::string xaddTrim(const std::string& key, const std::string& id, Input fst,
                       Input lst, uint32_t thr, bool apx = true) {
    try {
  ...
  int32_t keyslot(const std::string& key) {
    try {
      if (_cluster)
        return _cluster->command<long long>("cluster", "keyslot", key);
  ...
  //
  //  Note that this method will fail on a cluster unless src and dst hash to
  //  the same slot
  //    https://stackoverflow.com/questions/38042629/redis-cross-slot-error
  //    https://redis.io/docs/reference/cluster-spec/
  //
  int32_t copy(const std::string& src, const std::string& dst) {
    try {
      ...
      if (std::string(e.what()).find("CROSSSLOT") != std::string::npos)
        return -2;
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
    }
    return -1;
  ...
  bool rename(const std::string& src, const std::string& dst) {
    try {
      if (_cluster) {
        _cluster->rename(src, dst);
        return true;
      }
      if (_singler) {
        _singler->rename(src, dst);
        return true;
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
  ...
  std::vector<std::string> time(const std::string& key = "time") {
    ...
    try {
      if (_cluster)
        _cluster->redis(key, false).command("time", std::back_inserter(ret));
  ...
  //
  //    note - previously this function returned std::optional<swr::Subscriber>
  //    but
  //           std::optional has been replaced with swr::Optional to support
  //           c++11 and unfortunately swr::Optional cannot create an empty
  //           optional with swr::Subscriber - now client must delete new
  //           swr::Subscriber when done
  //
  swr::Subscriber* subscriber() {
    try {
      if (_cluster) {
        return new swr::Subscriber(_cluster->subscriber());
      }
      if (_singler) {
        return new swr::Subscriber(_singler->subscriber());
      }
    } catch (const swr::Error& e) {
      syslog(LOG_ERR, "RedisConnection::%s %s", __func__, e.what());
    }
