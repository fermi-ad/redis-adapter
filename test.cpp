...
TEST(RedisAdapter, DataSingle)
{
  ...
  EXPECT_TRUE(redis.addStreamSingle("abc", "xxx").ok());
  string s;
  EXPECT_TRUE(redis.getStreamSingle("abc", s).ok());
  EXPECT_STREQ(s.c_str(), "xxx");

  //  set/get float single element
  EXPECT_TRUE(redis.addStreamSingle("abc", 1.23f).ok());  //  Note value MUST have 'f' suffix else it's a double
  float f = 0;
  EXPECT_TRUE(redis.getStreamSingle("abc", f).ok());
  EXPECT_FLOAT_EQ(f, 1.23);                         //  Here it doesn't matter, we are just comparing

  //  set/get float single element
  EXPECT_TRUE(redis.addStreamSingle<float>("abc", 1.23).ok());  //  Here it's OK we are calling specialization <float>
  EXPECT_TRUE(redis.getStreamSingle("abc", f).ok());
  EXPECT_FLOAT_EQ(f, 1.23);                               //  Here it doesn't matter, we are just comparing

  //  set/get double single element
  EXPECT_TRUE(redis.addStreamSingleDouble("abc", 1.23).ok());
  double d = 0;
  EXPECT_TRUE(redis.getStreamSingle("abc", d).ok());
  EXPECT_DOUBLE_EQ(d, 1.23);

  //  set/get float vector single element
  vector<float> vf = { 1.23, 3.45, 5.67 };
  EXPECT_TRUE(redis.addStreamSingleList("abc", vf).ok());
  vf.clear();
  EXPECT_TRUE(redis.getStreamListSingle("abc", vf).ok());
  EXPECT_EQ(vf.size(), 3);
  ...
  EXPECT_TRUE(redis.addStreamSingleList("abc", ai).ok());
  //  note it comes back as a vector
  vector<int> vi;
  EXPECT_TRUE(redis.getStreamListSingle("abc", vi).ok());
  EXPECT_EQ(vi.size(), 3);
  EXPECT_EQ(vi[0], 1);
  EXPECT_EQ(vi[1], 2);
  EXPECT_EQ(vi[2], 3);
...
TEST(RedisAdapter, Data)
{
  ...
  //  set/get data
  RA_Time idA = redis.addStreamSingle("abc", "xxx");
  EXPECT_TRUE(idA.ok());
  RA_Time idB = redis.addStreamSingle("abc", "yyy");
  EXPECT_TRUE(idB.ok());
...
TEST(RedisAdapter, DataListener)
{
  ...
  EXPECT_TRUE(redis.addStreamSingleList("xyz", vf).ok());
...
TEST(RedisAdapter, Utility)
{
  ...
  int val = 1;
  EXPECT_TRUE(redis.addStreamSingle("srcdat", val).ok());
  EXPECT_TRUE(redis.copyStream("srcdat", "dstdat"));
  EXPECT_TRUE(redis.getStreamSingle("dstdat", val).ok());
  EXPECT_EQ(val, 1);
