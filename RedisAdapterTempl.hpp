...
//
template <typename T>
auto RedisAdapter::default_field_value(const Attrs& attrs) const {
  static_assert(std::is_trivial<T>(), "wrong type T");

  swr::Optional<T> ret;
  if (attrs.count(DEFAULT_FIELD))
    ret = *(const T*)attrs.at(DEFAULT_FIELD).data();
  return ret;
}
//  string specialization
template <>
inline auto RedisAdapter::default_field_value<std::string>(
...
//
template <typename T>
RedisAdapter::Attrs RedisAdapter::default_field_attrs(const T& data) const {
  static_assert(std::is_trivial<T>(), "wrong type T");
...
//  string specialization
template <>
inline RedisAdapter::Attrs RedisAdapter::default_field_attrs(
    const std::string& data) const {
  return {{DEFAULT_FIELD, data}};
}
//  overload for buffer as ptr, size
template <typename T>
RedisAdapter::Attrs RedisAdapter::default_field_attrs(const T* data,
                                                      size_t size) const {
  static_assert(std::is_trivial<T>(), "wrong type T");

  return {{DEFAULT_FIELD,
           data ? std::string((const char*)data, size * sizeof(T)) : ""}};
}

//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  get_forward_stream_helper : get data as type T (T is trivial, string or
//  Attrs)
...
//
template <typename T>
RedisAdapter::TimeValList<T> RedisAdapter::get_forward_stream_helper(
    const std::string& baseKey, const std::string& subKey, RA_Time minTime,
    RA_Time maxTime, uint32_t count) {
  static_assert(std::is_trivial<T>() || std::is_same<T, std::string>(),
                "wrong type T");
  ...
  if (count) {
    connect(_redis.xrange(key, minID, maxID, count, std::back_inserter(raw)));
  } else {
    connect(_redis.xrange(key, minID, maxID, std::back_inserter(raw)));
  }
...
template <>
inline RedisAdapter::TimeValList<RedisAdapter::Attrs>
RedisAdapter::get_forward_stream_helper(const std::string& baseKey,
...
//
template <typename T>
RedisAdapter::TimeValList<std::vector<T>>
RedisAdapter::get_forward_stream_list_helper(const std::string& baseKey,
                                             const std::string& subKey,
                                             RA_Time minTime, RA_Time maxTime,
                                             uint32_t count) {
  static_assert(std::is_trivial<T>(), "wrong type T");
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  get_reverse_stream_helper : get data as type T (T is trivial, string or
//  Attrs)
...
//
template <typename T>
RedisAdapter::TimeValList<T> RedisAdapter::get_reverse_stream_helper(
    const std::string& baseKey, const std::string& subKey, RA_Time maxTime,
    uint32_t count) {
  static_assert(std::is_trivial<T>() || std::is_same<T, std::string>(),
                "wrong type T");
  ...
  if (count) {
    connect(_redis.xrevrange(key, maxID, "-", count, std::back_inserter(raw)));
  } else {
    connect(_redis.xrevrange(key, maxID, "-", std::back_inserter(raw)));
  }

  TimeValList<T> ret;
  TimeVal<T> retItem;
  for (auto rawItem = raw.rbegin(); rawItem != raw.rend();
       rawItem++)  //  reverse iterate
  {
    swr::Optional<T> maybe = default_field_value<T>(rawItem->second);
    if (maybe) {
      retItem.first = RA_Time(rawItem->first);
...
template <>
inline RedisAdapter::TimeValList<RedisAdapter::Attrs>
RedisAdapter::get_reverse_stream_helper(const std::string& baseKey,
                                        const std::string& subKey,
                                        RA_Time maxTime, uint32_t count) {
  std::string key = build_key(subKey, baseKey);
  std::string maxID = maxTime.id_or_max();
...
template <typename T>
RedisAdapter::TimeValList<std::vector<T>>
RedisAdapter::get_reverse_stream_list_helper(const std::string& baseKey,
                                             const std::string& subKey,
                                             RA_Time maxTime, uint32_t count) {
  static_assert(std::is_trivial<T>(), "wrong type T");
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  get_single_stream_helper : get a single data item of type T (T is trivial,
//  string or Attrs)
//
//    baseKey : base key of device
//    subKey  : sub key to get data from
...
//
template <typename T>
RA_Time RedisAdapter::get_single_stream_helper(const std::string& baseKey,
                                               const std::string& subKey,
  ...
  ItemStream raw;
  connect(_redis.xrevrange(build_key(subKey, baseKey), maxTime.id_or_max(), "-",
                           1, std::back_inserter(raw)));
...
//  Attrs specialization
template <>
inline RA_Time RedisAdapter::get_single_stream_helper(
    const std::string& baseKey, const std::string& subKey, Attrs& dest,
    RA_Time maxTime) {
  ItemStream raw;
  connect(_redis.xrevrange(build_key(subKey, baseKey), maxTime.id_or_max(), "-",
                           1, std::back_inserter(raw)));
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  get_single_stream_list_helper : get a single data item as vector<T> (T is
//  trivial)
//
//    baseKey : base key of device
//    subKey  : sub key to get data from
...
//
template <typename T>
RA_Time RedisAdapter::get_single_stream_list_helper(const std::string& baseKey,
                                                    const std::string& subKey,
                                                    std::vector<T>& dest,
                                                    RA_Time maxTime) {
  static_assert(std::is_trivial<T>(), "wrong type T");
  ...
  if (raw.size()) {
    const std::string str =
        default_field_value<std::string>(raw.front().second);
    if (str.size()) {
...
//
template <typename T>
std::vector<RA_Time> RedisAdapter::addValues(const std::string& subKey,
  ...
  for (const auto& item : data) {
    ...
    std::string id =
        _redis.xadd(key, item.first.id_or_now(), attrs.begin(), attrs.end());

    if (id.size()) {
      ret.push_back(RA_Time(id));
    }
  }
  if (trim && ret.size()) {
    _redis.xtrim(key, std::max(trim, (uint32_t)ret.size()));
  }
...
//  Attrs specialization
template <>
inline std::vector<RA_Time> RedisAdapter::addValues(
    const std::string& subKey, const TimeValList<Attrs>& data, uint32_t trim) {
  std::vector<RA_Time> ret;
  std::string key = build_key(subKey);
  for (const auto& item : data) {
...
//
template <typename T>
std::vector<RA_Time> RedisAdapter::addLists(
    const std::string& subKey, const TimeValList<std::vector<T>>& data,
    uint32_t trim) {
  static_assert(std::is_trivial<T>(), "wrong type T");
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  addSingleValue : add a single data item of type T (T is trivial, string or
//  Attrs)
//
//    subKey : sub key to add data to
...
//
template <typename T>
RA_Time RedisAdapter::addSingleValue(const std::string& subKey, const T& data,
  ...
  std::string id =
      args.trim
          ? _redis.xaddTrim(key, args.time.id_or_now(), attrs.begin(),
                            attrs.end(), args.trim)
...
//  Attrs specialization
template <>
inline RA_Time RedisAdapter::addSingleValue(const std::string& subKey,
                                            const Attrs& data,
                                            const RA_ArgsAdd& args) {
  std::string key = build_key(subKey);
  ...
  std::string id =
      args.trim
          ? _redis.xaddTrim(key, args.time.id_or_now(), data.begin(),
                            data.end(), args.trim)
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  add_single_stream_list_helper : add a data item as buffer of type T data (T
//  is trivial)
//
//    subKey : sub key to add data to
...
//
template <typename T>
RA_Time RedisAdapter::add_single_stream_list_helper(const std::string& subKey,
                                                    RA_Time time, const T* data,
                                                    size_t size,
                                                    uint32_t trim) {
  static_assert(std::is_trivial<T>(), "wrong type T");
  ...
  std::string id =
      trim ? _redis.xaddTrim(key, time.id_or_now(), attrs.begin(), attrs.end(),
                             trim)
...
//
template <typename T>
RedisAdapter::reader_sub_fn RedisAdapter::make_reader_callback(
    ReaderSubFn<T> func) const {
  static_assert(std::is_trivial<T>() || std::is_same<T, std::string>(),
                "wrong type T");

  return [&, func](const std::string& base, const std::string& sub,
                   const ItemStream& raw) {
    TimeValList<T> ret;
    TimeVal<T> retItem;
    for (const auto& rawItem : raw) {
      swr::Optional<T> maybe = default_field_value<T>(rawItem.second);
...
//  Attrs specialization
template <>
inline RedisAdapter::reader_sub_fn RedisAdapter::make_reader_callback(
    ReaderSubFn<Attrs> func) const {
  return [&, func](const std::string& base, const std::string& sub,
                   const ItemStream& raw) {
    TimeValList<Attrs> ret;
    TimeVal<Attrs> retItem;
...
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
//  make_list_reader_callback : wrap user callback to convert data to vector of
//  T
//
...
//
template <typename T>
RedisAdapter::reader_sub_fn RedisAdapter::make_list_reader_callback(
