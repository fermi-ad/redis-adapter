...
All adapters, configs, and Docker Compose live under `adapters/`. A single `docker-compose.yml` (generated by `adapters/generate_system.py`) provides demo profiles, system simulation profiles, and TUI tools.

### Demo profiles

```bash
cd adapters
docker compose --profile bpm-demo up -d    # BPM twin + composite adapter
docker compose --profile blm-demo up -d    # BLM twin + adapter
docker compose --profile bcm-demo up -d    # BCM twin + adapter
docker compose --profile pipeline up -d    # individual signal-processing adapters
```

### System simulation

A full-scale accelerator simulation with **532 containers** (92 Redis + 220 twins + 220 adapters):
...
The TUIs crash when incorrectly detached, so they run interactively via `docker compose run`. All host files are mounted at `/etc/inst-tui/`.

```bash
cd adapters

# Instrument TUI — BPM orbit, BLM losses, BCM currents
docker compose --profile tui run --rm inst-tui                                                          # demo (localhost:6379)
docker compose --profile tui run --rm inst-tui /inst-tui --config /etc/inst-tui/inst-tui-hosts-tiny.txt  # system profile

# Redis stream inspector
docker compose --profile tui run --rm redis-tui /redis-tui redis-bpm-001:6379
```
...
redis-adapter/
  ...
  adapters/                Signal processing adapters (self-contained)
    docker-compose.yml     Unified compose (demos + system + TUIs)
    generate_system.py     Compose + system config generator
