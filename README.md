...
A full-scale accelerator simulation is provided via `compose/docker-compose.system.yml`, generated by `generate_system.py`. Each digitizer gets its own Redis instance. It runs **532 containers** (92 Redis + 220 twins + 220 adapters) simulating:

- **125 BPMs** (8 channels each, 500 IQ channels total) — 70 digitizers (1 Redis each)
- **130 BLMs** (1 channel each) — 17 digitizers (8 BLMs per Redis)
- **20 BCMs** (4 channels each, 80 channels total) — 5 digitizers (4 BCMs per Redis)
...
Five size profiles are available to run a subset of the system:
...
docker compose -f compose/docker-compose.system.yml --profile tiny up -d
docker compose -f compose/docker-compose.system.yml --profile large up -d
docker compose -f compose/docker-compose.system.yml --profile full up -d
```

### Instrument TUI

The `inst-tui` provides a real-time terminal dashboard for monitoring BPM orbit, BLM losses, and BCM currents. Run it on-demand against a running system:

```bash
docker run --rm -it --network redis-adapter_default \
  -v ./system-configs/inst-tui-hosts-large.txt:/etc/inst-tui/hosts.txt:ro \
  redis-adapter-bpm-001 /inst-tui --config /etc/inst-tui/hosts.txt
```

To inspect a single Redis instance with `redis-tui`:

```bash
docker run --rm -it --network redis-adapter_default \
  redis-adapter-bpm-001 /redis-tui redis-bpm-001:6379
```
